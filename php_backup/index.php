...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    try {
        ...
        if ($action === 'verify_code') {
            ...
            if (!preg_match('/^CARD-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/i', $code)) {
                echo json_encode(['success'=>false,'error'=>'激活码格式不正确']); exit;
            }
  ...
  <div class="max-w-2xl mx-auto px-4 py-8">
    ...
    <div id="step1" class="bg-white rounded shadow p-6 mb-6">
      ...
      <form id="formVerify" class="space-y-4">
        <input id="code" type="text" placeholder="CARD-XXXX-XXXX-XXXX" class="w-full border rounded px-3 py-2" required />
